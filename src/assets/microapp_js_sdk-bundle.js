...
goog.loadModule(function(exports) {'use strict';goog.module('boq.paisamicroappsjs.api.MicroappsApi');

/**
 * Interface for Microapps Javascript SDK APIs.
 * @interface
 */
class MicroappsApi {
  /**
   * Initializes microapps api client. Microapps should call this method first
   * for client initialization before calling any microapps apis.
   */
  initialize() {};

  /**
   * Executes the native flow for fulfilling a payment request.
   * <p>See go/microapps-payment.
   * @param {!PaymentDataRequest} paymentRequest - payment request for
   *     allowing the native host to start the payment interaction with the
   *     user.
   * @return {!Promise<!PaymentDataResponse>}
   */
   ...
   * @return {!Promise<!PaymentDataResponse>}
   */
  blockFund(paymentRequest) {}

  /**
   * Privileged: Executes the native flow for Cricket sharing.
   * See go/microapps-sharing-cricket.
   *
   * @param {!SharingForCricketDataRequest} sharingForCricketRequest - Cricket sharing request for
   *     allowing the native host to share to internal or external.
   * @return {!Promise<undefined>}
   */
  requestSharingForCricket(sharingForCricketRequest) {}

  /**
   * Executes the native flow for sharing.
   * See go/microapps-sharing.
   *
   * @param {!SharingDataRequest} sharingRequest - Sharing request for
   *     allowing the native host to share to internal or external.
   * @return {!Promise<undefined>}
   */
  requestSharing(sharingRequest) {}

  /**
   * Executes the native flow for Show reward.
   * See go/gpay-minigame-dd.
   *
   * @param {!ShowRewardDataRequest} showRewardRequest - Show reward request for
   *     allowing the native host to show reward in reward detail page.
   * @return {!Promise<undefined>}
   */
  requestShowReward(showRewardRequest) {}

  /**
   * Executes the native flow for scanning code.
   * <p>See go/microapps-scancode.
   * @param {!ScanCodeRequest=} scanRequest - scan code arguments for
   *     allowing the native host to open camera for the user to scan code.
   * @return {!Promise<string>}
   */
  requestScan(scanRequest) {}

  /**
   * Execute create order request flow.
   * see go/microapps-order
   * @param{!Order} order
  ...
  /**
   * Gets the current location of the current device.
   * See: go/microapps-location-api.
   * @return {!Promise<!GetCurrentLocationResponse>}
   */
  getCurrentLocation() {}

  /**
   * Privileged: Opens a whitelisted URL or another microapp.
   * <p>See: go/microapps-support-page.
   * @param {!OpenUrlRequest} openUrlRequest
   */
  openUrl(openUrlRequest) {}

  /**
   * Privileged: Executes the native flow for fulfilling first party payment
   * request. See go/microapps-1p-payment.
   * <p>Example param from Gold delivery microapp
   * {
   *   payeeFrontendActorId: "uGICAgICA-oHpCw",
   *   payeeProfileUrl:
   * "https://www.gstatic.com/nbupayments/merchant_logos/mmtc_pamp.png"
   *   payeeName: "MMTC"
   *   orderVerticalServiceMetadata: "bytestrig"
   *   amount: "12.45"
   *   currencyCode: "INR"
   *   paymenSuccessAnimation: "gold"
   *   shouldCloseAppAfterPayment: true
   * }
   * @param {!FirstPartyPaymentDataRequest} firstPartyPaymentRequest - payment
   *     request for allowing the native host to start the payment interaction
   *     with the user with optional custom data.
   * @return {!Promise<undefined>}
   */
  requestPaymentInternal(firstPartyPaymentRequest) {}

  /**
   * Privileged: Gets the request header needed for interacting with the Paisa
   * FE.
   * @return {!Promise<string>}
   */
  getRequestHeader() {}
  ...
  /**
   * Privileged: Gets the client metadata for risk assessment.
   * @return {!Promise<string>} the base64-encoded ClientMetadata
   *     (go/client_metadata_proto).
   */
  getClientMetadata() {}

  /**
   * Privileged: Returns a DroidGuard blob generated from the provided
   * droidGuardArgs.
   * @param {!Object<string, string>} droidGuardArgs
   * @return {!Promise<string>} the generated DroidGuard blob.
   */
  getDroidGuardResults(droidGuardArgs) {}

  /**
   * Privileged: Returns an Array of ExtractedTokenInfo’s extracted from an
   * incoming SMS. The tokens are extracted based on the regex provided. If the
   * desired SMS didn’t arrive within the desired timeout (seconds), will
   * reject(“Timeout reached.”).
   * @param {!SmsTokenRequest} smsTokenRequest
   * @return {!Promise<!Array<!ExtractedTokenInfo>>}
   */
  getSmsToken(smsTokenRequest) {}

  /** Privileged: Closes the Microapp. */
  closeApp() {}

  /**
   * Creates api proxy instance.
   * @param {!ApiProvider} provider
   * @return {!MicroappsApi}
   */
  static create(provider) {
    apiProvider = provider;
    return new MicroappsApiDelegate();
  }
}

/**
 * Provider for the Microapps Api.
 * @typedef {function(): !MicroappsApi}
 */
let ApiProvider;
...
/**
 * @implements {MicroappsApi}
 * @final
 * @package
 */
class MicroappsApiDelegate {
  /** @override */
  initialize() {
  ...
  /** @override */
  requestPayment(paymentRequest) {
    return apiProvider().requestPayment(paymentRequest);
  };

  /** @override */
  blockFund(paymentRequest) {
    return apiProvider().blockFund(paymentRequest);
  }

  /** @override */
  requestPaymentInternal(firstPartyPaymentRequest) {
    return apiProvider().requestPaymentInternal(firstPartyPaymentRequest);
  };

  /** @override */
  requestSharingForCricket(sharingForCricketRequest) {
    return apiProvider().requestSharingForCricket(sharingForCricketRequest);
  }

  /** @override */
  requestSharing(sharingRequest) {
    return apiProvider().requestSharing(sharingRequest);
  }

  /** @override */
  requestShowReward(showRewardRequest) {
    return apiProvider().requestShowReward(showRewardRequest);
  }

  /** @override */
  requestScan(scanRequest) {
    return apiProvider().requestScan(scanRequest);
  }

  /** @override */
  createOrder(order) {
    return apiProvider().createOrder(order);
  }

  /** @override */
  openUrl(url) {
    apiProvider().openUrl(url);
  }

  /** @override */
  getRequestHeader() {
    return apiProvider().getRequestHeader();
  }

  /** @override */
  getClientMetadata() {
    return apiProvider().getClientMetadata();
  }

  /** @override */
  getDroidGuardResults(droidGuardArgs) {
    return apiProvider().getDroidGuardResults(droidGuardArgs);
  }

  /** @override */
  getSmsToken(smsTokenRequest) {
    return apiProvider().getSmsToken(smsTokenRequest);
  }

  /** @override */
  closeApp() {
    apiProvider().closeApp();
  }

  /** @override */
  getCurrentLocation() {
    return apiProvider().getCurrentLocation();
  }
}

exports = MicroappsApi;

;return exports;});

//java/com/google/nbu/paisa/microapps/js/api/config.js
goog.loadModule(function(exports) {'use strict';/** @fileoverview Configs for the microapps SDK. */

goog.module('boq.paisamicroappsjs.api.config');

/**
 * Enables the privileged api in the microapps SDK.
 * <p>Default is set to true for google3 project, as Boq Web does not allow
 * compile flag to be overwritten easily. See
 * https://yaqs.googleplex.com/eng/q/5126514062131200 for more context.
 * <p>To overwrite it, use `--define=ENABLE_MICROAPPS_PRIVILEGE_API=false` in
 * the `defs` of the `js_binary` BUILD target.
 * @define {boolean}
 */
exports.ENABLE_MICROAPPS_PRIVILEGE_API =
    goog.define('ENABLE_MICROAPPS_PRIVILEGE_API', true);

;return exports;});

//java/com/google/nbu/paisa/microapps/js/messaging/constants.js
goog.loadModule(function(exports) {'use strict';goog.module('boq.paisamicroappsjs.messaging.constants');

/**
 * Message names used in messaging between viewer and the microapp.
 *
 * @const @enum {string}
 */
exports.MessageName = {
 ...
 *
 * <p>IMPORTANT: It's a strict requirement that any non-prod origin added
 * here is not reachable through public internet. This is so that if a microapp
 * is loaded from outside GPay/WebView, it cannot use a non-prod viewer.
 *
 * <p>Domains under .sandbox.google.com are by default accessible only by debug
 * IPs. Any change in the default access control should preserve this property.
 * See
 * https://g3doc.corp.google.com/production/sandbox_services/g3doc/index.md?cl=head#access-control.
 *
 * @type {!RegExp}
 */
exports.AllowedViewerOrigin =
    /^https:\/\/microapps(-(autopush|preprod|prod-tt|daily-[0-6])\.sandbox)?\.google\.com$/;
...
goog.loadModule(function(exports) {'use strict';goog.module('boq.paisamicroappsjs.messaging.Messaging');

const {AllowedViewerOrigin, MessageName} = goog.require('boq.paisamicroappsjs.messaging.constants');
const {Logger, error, getLogger, info} = goog.require('goog.log');
...
/**
 * @typedef {function(!MicroappsViewerMessage):(!Promise<*>|undefined)}
 */
let RequestHandler;

/**
 * Checks if the window is inside an iframe.
 *
 * Ref: http://google3/maps/api/javascript/util/base.js?l=591&rcl=181249686
 *
 * @return {boolean}
 */
const isWindowInIframe = (/** !Window */ win) => {
  try {
...
/**
 * Gets the viewer origin from the ancestor frames.
 * @param {!Window} win
 * @return {string}
 * @throws {!Error} If the environment is unknown or unsupported.
 */
const getViewerOrigin = (win) => {
  // This error should never throw for Android API 21+ and iOS 9+.
  if (!win.location.ancestorOrigins || !win.location.ancestorOrigins.length) {
    throw new Error('unsupported environment');
  }

  // We only inspect origin for the immediate parent frame.
  const parentOrigin = win.location.ancestorOrigins[0];

  if (AllowedViewerOrigin.test(parentOrigin)) {
    return parentOrigin;
  }

  throw new Error('unknown viewer origin');
};

/**
 * Helper class for getting messaging instance.
 */
class MessagingManager {
  /**
   * Starts connection with the viewer and sends the message port to the viewer.
   * @param {!Window} win
   * @param {!MessageChannel} messageChannel
   * @return {!Messaging}
   */
  static connectWithViewer(win, messageChannel) {
    // Currently, we only allow loading the Microapp in an iframe.
    if (!isWindowInIframe(win)) {
      throw new Error('Unsupported environment: expected running in iframe');
    }

    const localPort = messageChannel.port1;
    const messaging =
        new Messaging(localPort, /** waitForConnectEnabled= */ true);

    // Using window.postMessage to transfer the port to the viewer (parent
    // iframe).
    let connectionRequestMessage = /** @type {!MicroappsViewerMessage} */ ({
      type: MessageType.REQUEST,
      name: MessageName.REQUEST_CONNECTION,
    });
    const targetOrigin = getViewerOrigin(win);
    win.parent.postMessage(
        connectionRequestMessage, targetOrigin, [messageChannel.port2]);
    return messaging;
  }
}

/**
 * Class for handling messaging between the microapp and the Microapps viewer.
 */
class Messaging {
  /**
   * @param {!MessagePort} port
   * @param {boolean=} waitForConnectEnabled If true, all request sending
   *     will be held until the connection is established (e.g. when microap try
   * to initialize the connection). Otherwise, the request will be sent
   * immediately to the channel (e.g. when viewer receives the port and is ready
   * to send message).
   */
  constructor(port, waitForConnectEnabled = false) {
    /** @const @private {!MessagePort} */
    this.port_ = port;
    ...
    /**
     * Maps a sent request with its response handler.
     *
     * @private @const {!Map<number, {resolve: function(*), reject:
     *     function(!Error)}>}
     */
    ...
    /** @private {function()} */
    this.waitForConnectResolver_;

    /** @const @private {?Logger} */
    this.logger_ = getLogger('boq.paisamicroappsjs.Messaging');

    /** @const @private {!Promise<*>} */
    this.waitForConnectPromise_ =
        new Promise((resolve) => this.waitForConnectResolver_ = resolve);
  ...
  /**
   * Handles message received in the channel. Dispatches message to its handler
  ...
  handleMessage_(event) {
    ...
    const message = /** @type {!MicroappsViewerMessage} */ (messageEvent.data);
    if (!message) {
      return;
    }

    // TODO(b/122807089): better validate message format
    if (!goog.isDefAndNotNull(message.id)) {
      error(this.logger_, 'Invalid message: expected field "id" not found.');
      return;
    }
  ...
  /**
   * Handles message of Request type received in the channel.
   *
   * @param {!MicroappsViewerMessage} message
   * @private
   */
  handleRequest_(message) {
    // Special handling when the viewer accept the connection.
    if (message.name === MessageName.ACCEPT_CONNECTION) {
      info(this.logger_, 'Viewer connection accepted.');
      this.waitForConnectResolver_();
      return;
    }
    if (!this.messageHandler_) {
      error(
          ...
          `Failed to handle message: ${
              message.name}, no handler is available.`);
      return;
    }
    const promise = this.messageHandler_(message);
    if (message.isResponseRequired) {
      const requestId = message.id;
      if (!promise) {
        error(
            this.logger_, `expected response but none given: ${message.name}`);
      ...
      promise.then(
          ...
          (failure) => {
            this.sendResponseError_(requestId, message.name, failure);
          });
    }
  }

  /**
   * Handles message of Response type received in the channel.
   *
   * @param {!MicroappsViewerMessage} message
   * @private
   */
  handleResponse_(message) {
    const requestId = message.id;
    const pendingRequest = this.sentMessages_.get(requestId);
    // Resolve the pending request if it exists.
    if (pendingRequest) {
      this.sentMessages_.delete(requestId);
      if (message.error) {
  ...
  sendResponse_(requestId, messageName, messageData) {
    this.sendMessage_(/** @type {!MicroappsViewerMessage} */ ({
      id: requestId,
  ...
  /**
   * Sends error response for a request.
   *
   * @param {number} requestId
   * @param {string} messageName
   * @param {*} error
   * @private
   */
  sendResponseError_(requestId, messageName, error) {
    const errString = error ? (error instanceof Error ? String(error) : error) :
                              'Unknown error';
    info(
        this.logger_,
        `sendResponseError_, Message name: ${messageName}, ${errString}`);
    this.sendMessage_(/** @type {!MicroappsViewerMessage} */ ({
      id: requestId,
      type: MessageType.RESPONSE,
      name: messageName,
  ...
  /**
   * Sends the message to the receiver via the message port.
   *
   * @param {!MicroappsViewerMessage} message
   * @private
   */
  ...
  /**
   * Sends a request to the other side of message channel.
   *
   * @param {string} messageName
   * @param {*} messageData
   * @param {boolean=} requiresResponse default true. Indicates if the sender is
   *     expecting a response from the receiver.
   * @return {!Promise<*|undefined>} if requiresResponse is set to false, this
   *     method will returns a immediate Promise resolved with undefined (which
   *     can be ignored by the caller).
   */
  sendRequest(messageName, messageData, requiresResponse = true) {
    const requestId = ++this.requestIdCounter_;
    let promise = Promise.resolve();
    if (requiresResponse) {
      promise = new Promise((resolve, reject) => {
        this.sentMessages_.set(requestId, {resolve, reject});
      });
    }
    let messageToSent = /** @type {!MicroappsViewerMessage} */ ({
      id: requestId,
      type: MessageType.REQUEST,
      name: messageName,
      data: messageData,
      isResponseRequired: requiresResponse,
    });
    if (this.waitForConnectEnabled_) {
      this.waitForConnectPromise_.then(() => {
  ...
  registerHandler(requestHandler) {
    this.messageHandler_ = requestHandler;
  }
...
goog.loadModule(function(exports) {'use strict';goog.module('boq.paisamicroappsjs.api.v1.MicroappsApiImpl');

const MicroappsApi = goog.require('boq.paisamicroappsjs.api.MicroappsApi');
...
const {ENABLE_MICROAPPS_PRIVILEGE_API} = goog.require('boq.paisamicroappsjs.api.config');
const {Logger, error, getLogger} = goog.require('goog.log');
const {Messaging, MessagingManager} = goog.require('boq.paisamicroappsjs.messaging.Messaging');
...
/**
 * Holds message channel instance.
 * There is a bug on Safari 11.1: If message channel is not retained, it will be
 * deallocated and all the messages sent through this channel will be lost.
 * Please refer to https://bugs.webkit.org/show_bug.cgi?id=184502.
 * @type {?MessageChannel}
 */
 let messageChannel = null;

/**
 * Microapps API implementation v1.
 *
 * @implements {MicroappsApi}
 * @final
 */
class MicroappsApiImpl {
  /**
   * Gets the singleton of Microapp API implementation instance. Connect to the
   * viewer that loads the current microapp.
   * @return {!MicroappsApi}
   */
  static getInstance() {
    if (!instance) {
      messageChannel = new MessageChannel();
      instance = new MicroappsApiImpl(
          window,
          MessagingManager.connectWithViewer(window, messageChannel));
    }
    return /** @type {!MicroappsApi} */ (instance);
  }

  /**
   * @param {!Window} win the window object.
   * @param {!Messaging} messaging viewer <=> microapp messaging interface.
   * @package
   */
  constructor(win, messaging) {
    /** @private @const {!Messaging} */
    ...
    this.logger_ = getLogger('boq.paisamicroappsjs.v1.apiimpl');
  }

  /** @override */
  initialize() {
    if (!this.isLoadedInTrustedOrigins_()) {
      // Don't render the page if it's embedded in untrusted domain.
      safeDom.setLocationHref(
          googAsserts.assert(this.win_.location), SafeUrl.ABOUT_BLANK);
    }
  }

  /** @override */
  requestPayment(paymentRequest) {
    if (paymentRequest == null || !goog.isObject(paymentRequest)) {
      return Promise.reject(
          'valid paymentRequest object is required for this API.');
    }

    return /** @type {!Promise<!PaymentDataResponse>} */ (
        this.messaging_.sendRequest('requestPayment', paymentRequest));
  }

  /** @override */
  requestPaymentInternal(firstPartyPaymentRequest) {
    checkPrivilegedApiAccess();

    if (firstPartyPaymentRequest == null ||
        !goog.isObject(firstPartyPaymentRequest)) {
      return Promise.reject(
          'Valid firstPartyPaymentRequest object is required');
    }

    return /** @type {!Promise<undefined>} */ (this.messaging_.sendRequest(
        'requestPaymentInternal', firstPartyPaymentRequest));
  }
  ...
  blockFund(paymentRequest) {
    ...
    return /** @type {!Promise<!PaymentDataResponse>} */ (
        this.messaging_.sendRequest('blockFund', paymentRequest));
  }
  ...
  requestScan(scanRequest) {
    ...
    return /** @type {!Promise<string>} */ (
        this.messaging_.sendRequest('requestScan', request));
  }
  ...
  requestSharingForCricket(sharingForCricketRequest) {
    ...
    if (sharingForCricketRequest === null ||
        !goog.isObject(sharingForCricketRequest)) {
      return Promise.reject(
          'valid sharingForCricketRequest object is required for this API.');
    }
    return /** @type {!Promise<undefined>} */ (this.messaging_.sendRequest(
        'requestSharingForCricket', sharingForCricketRequest));
  }
  ...
  /** @override */
  requestSharing(sharingRequest) {
    if (sharingRequest == null || !goog.isObject(sharingRequest)) {
      return Promise.reject(
          'valid sharingRequest object is required for this API.');
    }
    return /** @type {!Promise<undefined>} */ (
        this.messaging_.sendRequest('requestSharing', sharingRequest));
  }
  ...
  requestShowReward(showRewardRequest) {
    ...
    if (showRewardRequest == null ||
        !goog.isObject(showRewardRequest)) {
      return Promise.reject(
          'valid showRewardRequest object is required for this API.');
    }
    return /** @type {!Promise<undefined>} */ (this.messaging_.sendRequest(
        'requestShowReward', showRewardRequest));
  }
  ...
  createOrder(order) {
    if (order == null) {
      return Promise.reject('valid order object is required for this API.');
    }

    return /** @type {!Promise<!Order>} */ (
        this.messaging_.sendRequest('createOrder', order));
  }

  /** @override */
  getCurrentLocation() {
    return /** @type {!Promise<!GetCurrentLocationResponse>} */ (
        this.messaging_.sendRequest('getCurrentLocation', {}));
  }
  ...
  openUrl(openUrlRequest) {
    ...
    this.messaging_.sendRequest('openUrl', openUrlRequest)
        .catch(
            (reason) => error(this.logger_, googAsserts.assertString(reason)));
  }
  ...
  getRequestHeader() {
    ...
    return /** @type {!Promise<string>} */ (
        this.messaging_.sendRequest('getRequestHeader', {}));
  }
  ...
  getClientMetadata() {
    ...
    return /** @type {!Promise<string>} */ (
        this.messaging_.sendRequest('getClientMetadata', {}));
  }
  ...
  getDroidGuardResults(droidGuardArgs) {
    ...
    return /** @type {!Promise<string>} */ (
        this.messaging_.sendRequest('getDroidGuardResults', droidGuardArgs));
  }
  ...
  getSmsToken(smsTokenRequest) {
    ...
    if (smsTokenRequest.fixed.length < 20) {
      return Promise.reject(
          'Fixed string must be at least 20 characters long.');
    }
    if (smsTokenRequest.timeout > 300) {
      return Promise.reject('Exceeded maximum timeout.');
    }


    return /** @type {!Promise<!Array<!ExtractedTokenInfo>>} */ (
        this.messaging_.sendRequest('getSmsToken', smsTokenRequest));
  }
  ...
  closeApp() {
    ...
    this.messaging_.sendRequest('closeApp', {});
  }

  /**
   * Checks whether the microapp iframe is loaded in trusted origin.
   * This is for clickjacking protection. See #15 in b/123051400.
   * @return {boolean} whether the iframe parent origin is 1P domain.
   * @private
   */
  isLoadedInTrustedOrigins_() {
    if (this.win_.self !== this.win_.top) {
      if (this.win_.location.origin && this.win_.location.ancestorOrigins) {
        let appOrigin = this.win_.location.origin;
        let parentOrigins = this.win_.location.ancestorOrigins;
        return googArray.every(parentOrigins, function(origin) {
          // Only trust google domain and microapp host domain.
          let domain = uriUtils.getDomain(origin) || '';
          return googString.caseInsensitiveEndsWith(domain, '.google.com') ||
              origin === appOrigin;
        });
      }
      return false;
...
/**
 * Checks the access to privileged api.
 * @throws {!Error} In case the privileged API is not accessible. i.e. The flag
 *     ENABLE_MICROAPPS_PRIVILEGE_API is false.
 * @private
 */
function checkPrivilegedApiAccess() {
  if (!ENABLE_MICROAPPS_PRIVILEGE_API) {
    throw Error('api not accessible');
  }
}

exports = MicroappsApiImpl;

;return exports;});

//java/com/google/nbu/paisa/microapps/js/api/api_exports.js
goog.loadModule(function(exports) {'use strict';/** @fileoverview Export symbols for Microapps SDK APIs. */

goog.module('boq.paisamicroappsjs.api.exports');

const MicroappsApi = goog.require('boq.paisamicroappsjs.api.MicroappsApi');
const MicroappsApiImpl = goog.require('boq.paisamicroappsjs.api.v1.MicroappsApiImpl');

/**
 * Holds the singleton api instance.
 * @type {!MicroappsApi}
 */
const api = MicroappsApi.create(() => MicroappsApiImpl.getInstance());

// Export the APIs to be used in 3P microapps.
goog.exportSymbol('microapps.requestPayment', api.requestPayment);
goog.exportSymbol('microapps.blockFund', api.blockFund);
goog.exportSymbol('microapps.requestScan', api.requestScan);
goog.exportSymbol('microapps.createOrder', api.createOrder);
goog.exportSymbol('microapps.initialize', api.initialize);
goog.exportSymbol('microapps.getCurrentLocation', api.getCurrentLocation);
goog.exportSymbol('microapps.requestSharing', api.requestSharing);
